...
int main() {
    ...
    if (!asignado) {
        std::cout << "Advertencia: La seccion \"" << sec.name 
                  << "\" del profesor " << sec.professor.name 
                  << " no recibio horario por falta de espacio.\n";
        ++sinHorario;
    }
}
if (sinHorario > 0) {
    std::cout << "\nTotal de secciones sin horario: " << sinHorario << "\n";
}
